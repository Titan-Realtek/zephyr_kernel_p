...
typedef struct rtk_i3c_core {
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t pcr: 1;     /**< Indicate the ctroller is primary or secondary */
    ...
    union {
        ...
        struct {
            volatile uint32_t cstall: 1;   /**< Enable Role TAGT to stretch SCL in I2C BUS. */
            volatile uint32_t i2c: 1;      /**< Indicate the protocol is I2C. */
            volatile uint32_t hdex_dis: 1; /**< Disable Role CTRL to send HDR EXIT Pattern
                                              after receiving NACKed after it sended
                                              broadcase write (7E/W). */
            volatile uint32_t ft_od: 1;    /**< Enable Role CTRL to force Timing =
                                              Open-Drain (OD) mode. */
            volatile uint32_t hder_pp: 1;  /**< Enable Role CTRL to send header address with
                                              Push-Pull (PP) mode. */
            volatile uint32_t nak_dis: 1;  /**< Disable RX NACK detection for Role CTRL. */
            volatile uint32_t ce_dis: 1;   /**< Disable SDR error detction for Role CTRL. */
            volatile uint32_t te_dis: 1;   /**< Disable SDR error detector for Role TAGT. */
            ...
            volatile uint32_t blen: 1;      /**< Enable Burst Length (BL) function when INTEL.Spec
                                               and PEC are enabled. */
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t sda_dly: 4; /**< Set SDA delay count after SCL negedge for Role
                                             CTRL to send TX data. */
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t tdfltr: 6; /**< Indicate the 50ns Spike Filter in
                                            I2C TAGT device. */
            volatile uint32_t dfltr: 1;  /**< Enable SDA spike filter in I2C TAGT device. */
            volatile uint32_t cfltr: 1;  /**< Enable SCL spike filter for I2C TATG device. */
            uint32_t _reserved0: 4;
            volatile uint32_t crc_epush: 1; /**< Enable CRC byte push to RXDA FIFO function
                                               in I3C private write transfer. */
            volatile uint32_t crc_evalc: 1; /**< Enable CRC byte value translated function in
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t astart: 1; /**< Control structure start async mode0 flow. */
            ...
            volatile uint32_t flush: 1;  /**< Flush BYFM/TXDA/RXDA and recovery ixc status. */
        } cstr_b;
    };
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t bdr_fst: 8; /**< Baud rate for the first header address in I3C
                                             BUS for Role CTRL. */
        } cbdr_b;
    };
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t abusy: 1;  /**< Indicate the current status for Async mode0. */
            volatile uint32_t amode: 1;  /**< Indicate that RXI356_I3CC is in Async mode0. */
            ...
            volatile uint32_t ccc: 1;   /**< Indicate that Role TAGT has received a CCC cmd. */
            volatile uint32_t ddr: 1;   /**< Indicate that RXI356_I3CC is in HDR-DDR mode. */
            uint32_t _reserved2: 10;
            volatile uint32_t rts_ptn: 1; /**< Indicate that Role TAGT has received an Target
                                             Reset Pattern. */
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t rxdp: 1;     /**< Indicate the RX Data Drop status
                                              for Role TAGT. */
            volatile uint32_t ecom: 1;     /**< Indicate the transaction status for Role CTRL. */
            ...
            volatile uint32_t rxibi: 1;    /**< Indicate IBI request status for Role CTRL. */
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t ccc: 1;    /**< Indicate that Role TAGT has received
                                            an CCC CMD. */
            volatile uint32_t rxinf: 16; /**< Indicate the current transaction status. */
        } trinf_b;
    };
    ...
    union {
        ...
        struct {
            volatile uint32_t tbuf: 12; /**< Bus Free Timing (tBUF) count for Mixed Bus. */
    ...
    union {
        ...
        struct {
            volatile uint32_t tsta_to: 32; /**< Timeout count for pulling SCL=Low when SDA is
                                              Low at SEC HP flow. */
        } tstmr_b;
    };
    ...
    union {
        ...
        struct {
            volatile uint32_t tcrhpovlp: 16; /**< tCRHPOverlap timing count. After this time
                                                delay, the Former active controller should
                                                release SCL to High-Z. */
            volatile uint32_t tncrl: 16;     /**< tNEWCRLock timing count. During this time
                                                interval, the New-controller shall not drive SDA
                                                Low. */
        } thpr_b;
    };
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t pdly: 8;    /**< Indicate the delay count for RX SDA sampling
                                             point in address header/data phase with PP mode. */
            volatile uint32_t ody: 14;    /**< Indicate the delay count for RX SDA sampling
            ...
            volatile uint32_t rdge: 1;    /**< Indicate sampling point (edge) for Role CTRL/TAGT
                                             to capture RX SDR data */
        } tdlyr_b;
    };
    ...
    union {
        ...
        struct {
            volatile uint32_t pre_dly: 8;  /**< The timing for SCL keep in low before preamble
                                              phase in DDR mode (min: BDR_PP). */
            volatile uint32_t ddr_pdly: 7; /**< SDA Received delay count for DDR mode. */
    ...
    union {
        ...
        struct {
            volatile uint32_t cetm_min_len: 8;   /**< Minimum length of transaction not to be
                                                    early terminated. */
            volatile uint32_t cetm_max_len: 8;   /**< Maximum length of transaction to be early
                                                    terminated. */
            ...
            volatile uint32_t intr_etm: 1;       /**< Previous transaction has been
                                                    early terminated. */
            volatile uint32_t intr_disec: 1;     /**< BUS arbitration won by TAGT not known to be
                                                    Monitoring Device. */
            volatile uint32_t disec_addr: 7;     /**< Dynamic address of TAGT which
                                                    won BUS arbitration. */
            volatile uint32_t metm_regin_cnt: 3; /**< Delay cycle of the timing point for
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t clk_wrap: 1;  /**< Clock gate enable of apb_wrapper module. */
            volatile uint32_t clk_ixc: 1;   /**< Clock gate enable of ixc_controller module. */
            volatile uint32_t clk_bfifo: 1; /**< Clock gate enable of byfm_fifo module. */
            volatile uint32_t clk_rfifo: 1; /**< Clock gate enable of rxda_fifo module. */
            volatile uint32_t clk_tfifo: 1; /**< Clock gate enable of txda_fifo module. */
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t daa_ing: 1;  /**< I3C.dynamic address assignment
                                              flow is going */
            volatile uint32_t daa_done: 1; /**< I3C.dynamic address assignment is done */
        } dsa_b;
    };
    ...
    union {
        ...
        struct {
            volatile uint32_t c_ref: 16; /**< Controller counter from IBI start to the first
                                            SCL positive-going edge after the IBI ACK */
            volatile uint32_t c_c2: 8;   /**< Controller counter from the first SCL
                                            positive-going edge after the IBI ACK to the first
                                            SCL positive-going edge after the T-Bit of the
                                            IBI's MDB */
            uint32_t _reserved0: 6;
            volatile uint32_t intr_asyn: 1; /**< Indicate ASYN IBI finished and inform user
                                               to read CASYN and TASYN */
    ...
    union {
        ...
        struct {
            volatile uint32_t t_c1: 16; /**< Target counter from IBI start to the first SCL
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t wr_nack: 1; /**< Enable ACK/NACK Capability for
                                             WRITE Command */
            volatile uint32_t wr_etm: 1;  /**< Enable HDR WRITE Early Termination Request */
    ...
    union {
        ...
        struct {
            volatile uint32_t mxds_rt: 24; /**< 3-byte MaxRdTurn time in us to tell
                                              controller how long to wait before reading the
                                              data it requested */
    ...
    union {
        ...
        struct {
            ...
            volatile uint32_t rstact2: 8; /**< Return time in second for CCC.RSTACT direct
                                             get with def_byte = 0x82 */
            uint32_t _reserved1: 8;
            volatile uint32_t rstact3: 8; /**< Reserved */
            uint32_t _reserved0: 8;
            volatile uint32_t rstact0: 8; /**< For Direct SET: Indicate the previous received
                                             defining byte (contain 0x0-0x4). For Direct GET:
                                             Return byte for CCC.RSTACT direct get with
                                             def_byte = 0x0-0x3, 0x80 */
        } rstact_b;
    };
    ...
    union {
        ...
        struct {
            uint32_t _reserved0: 4;
            volatile const uint32_t asyn: 1; /**< ASYN bit - Raw IBI async_mode 0 status */
            uint32_t _reserved1: 3;
            volatile const uint32_t etm: 1; /**< ETM bit - Raw early termination status */
            uint32_t _reserved2: 22;
            volatile const uint32_t cmse: 1; /**< CMSE bit - Raw data size error status */
        } risr1_b;
    };
...
{
    LOG_REG("header=%" PRIu32 ", rdwr=%" PRIu32 ", stop=%" PRIu32 ", ftod=%" PRIu32 ", ack=%" PRIu32
            ", tx9bit=%" PRIu32 ", rx9bit=%" PRIu32 ", hdr=%" PRIu32 ", seq=%" PRIu32
            ", bcnt=%" PRIu32 ", ndf=%" PRIu32 ", ptxc=%" PRIu32 ", prxc=%" PRIu32 ", end=%" PRIu32
            "\n",
