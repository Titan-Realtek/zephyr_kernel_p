...
#define DT_DRV_COMPAT realtek_rts5918_bbram
...
static int bbram_rts5918_read(const struct device *dev, size_t offset, size_t size, uint8_t *data)
{
	const struct bbram_rts5918_config *config = dev->config;
	...
	/* The bbram in RTS5918 only accepts word-access.
...
static int bbram_rts5918_write(const struct device *dev, size_t offset, size_t size,
			       const uint8_t *data)
{
	const struct bbram_rts5918_config *config = dev->config;
	...
	/* The BBRAM in RTS5918 only accepts word-access. Read it as a word,
...
static const struct bbram_driver_api bbram_rts5918_driver_api = {
	.get_size = bbram_rts5918_get_size,
	.read = bbram_rts5918_read,
	.write = bbram_rts5918_write,
};

#define BBRAM_INIT(inst)                                                                           \
	static const struct bbram_rts5918_config bbram_cfg_##inst = {                              \
