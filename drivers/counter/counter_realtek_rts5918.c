...
#define DT_DRV_COMPAT realtek_rts5918_timer

/**
 * @file
 * @brief Realtek RTS5918 Counter driver
 *
...
LOG_MODULE_REGISTER(counter_realtek_rts5918, 4);//CONFIG_COUNTER_LOG_LEVEL);
struct counter_rts5918_config {
...
struct counter_rts5918_data {
	counter_alarm_callback_t alarm_cb;
	counter_top_callback_t top_cb;
	void *user_data;
};

#define COUNTER_RTS5918_REG_BASE(_dev)                                                             \
	((const struct counter_rts5918_config *const)_dev->config)->base_address

static int counter_rts5918_start(const struct device *dev)
{
	const struct counter_rts5918_config *config = dev->config;
...
static int counter_rts5918_stop(const struct device *dev)
{
	const struct counter_rts5918_config *config = dev->config;
	volatile struct timer32_type *counter = config->base_address;
	...
	counter->intsts |= TIMER32_INTSTS_STS;
...
static int counter_rts5918_get_value(const struct device *dev, uint32_t *ticks)
{
	const struct counter_rts5918_config *config = dev->config;
	volatile struct timer32_type *counter = config->base_address;
...
static int counter_rts5918_set_alarm(const struct device *dev, uint8_t chan_id,
				     const struct counter_alarm_cfg *alarm_cfg)
{
	struct counter_rts5918_data *data = dev->data;
	const struct counter_rts5918_config *counter_cfg = dev->config;
	...
	counter->ctrl &= ~TIMER32_CTRL_EN;
	/* disable interrupt */
	counter->ctrl &= ~TIMER32_CTRL_INTEN_EN;
...
static int counter_rts5918_cancel_alarm(const struct device *dev, uint8_t chan_id)
{
	struct counter_rts5918_data *data = dev->data;
	const struct counter_rts5918_config *config = dev->config;
	volatile struct timer32_type *counter = config->base_address;
...
static uint32_t counter_rts5918_get_pending_int(const struct device *dev)
{
	const struct counter_rts5918_config *config = dev->config;
	volatile struct timer32_type *counter = config->base_address;
...
static uint32_t counter_rts5918_get_top_value(const struct device *dev)
{
	const struct counter_rts5918_config *config = dev->config;
	volatile struct timer32_type *counter = config->base_address;
...
static int counter_rts5918_set_top_value(const struct device *dev,
...
static void counter_rts5918_isr(const struct device *dev)
{
	struct counter_rts5918_data *data = dev->data;
	const struct counter_rts5918_config *config = dev->config;
	volatile struct timer32_type *counter = config->base_address;
...
static uint32_t counter_rts5918_get_freq(const struct device *dev)
{
	const struct counter_rts5918_config *counter_cfg = dev->config;
...
// static DEVICE_API(counter, counter_rts5918_api) = {
static const struct counter_driver_api counter_rts5918_api = {
	.start = counter_rts5918_start,
	.stop = counter_rts5918_stop,
	.get_value = counter_rts5918_get_value,
	.set_alarm = counter_rts5918_set_alarm,
	.cancel_alarm = counter_rts5918_cancel_alarm,
	.set_top_value = counter_rts5918_set_top_value,
	.get_pending_int = counter_rts5918_get_pending_int,
	.get_top_value = counter_rts5918_get_top_value,
	.get_freq = counter_rts5918_get_freq,
};

static int counter_rts5918_init(const struct device *dev)
{
	const struct counter_rts5918_config *counter_cfg = dev->config;
	volatile struct timer32_type *counter = counter_cfg->base_address;
	int rc;
	struct rts5918_sccon_subsys sccon_subsys = {
...
#define COUNTER_RTS5918_INIT(inst)                                                                 \
	static void counter_rts5918_irq_config_##inst(void);                                       \
                                                                                                   \
	static struct counter_rts5918_data counter_rts5918_dev_data_##inst;                        \
                                                                                                   \
	static struct counter_rts5918_config counter_rts5918_dev_config_##inst = {                 \
		.info =                                                                            \
			{                                                                          \
				...
				.channels = 1,                                                     \
                                                                                                   ...
                                                                                                   \
		.config_func = counter_rts5918_irq_config_##inst,                                  \
		.base_address = (struct timer32_type *)DT_INST_REG_ADDR(inst),                     \
		.prescaler = DT_INST_PROP(inst, prescaler),                                        \
		DEV_CONFIG_CLK_DEV_INIT(inst)};                                                    \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, counter_rts5918_init, NULL, &counter_rts5918_dev_data_##inst,  \
			      &counter_rts5918_dev_config_##inst, PRE_KERNEL_1,                    \
			      CONFIG_COUNTER_INIT_PRIORITY, &counter_rts5918_api);                 \
                                                                                                   \
	static void counter_rts5918_irq_config_##inst(void)                                        \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(inst), DT_INST_IRQ(inst, priority), counter_rts5918_isr,  \
			    DEVICE_DT_INST_GET(inst), 0);                                          \
		irq_enable(DT_INST_IRQN(inst));                                                    \
	}

DT_INST_FOREACH_STATUS_OKAY(COUNTER_RTS5918_INIT)
