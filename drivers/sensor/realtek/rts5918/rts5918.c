...
#define DT_DRV_COMPAT realtek_rts5918_tach
...
LOG_MODULE_REGISTER(tach_rts5918, CONFIG_SENSOR_LOG_LEVEL);

struct tach_rts5918_config {
...
struct tach_rts5918_data {
	uint16_t count;
};

#define COUNT_100KHZ_SEC  100000U
#define SEC_TO_MINUTE     60U
#define PIN_STUCK_TIMEOUT (1U)

static uint16_t prev_trash = 1;

int tach_rts5918_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	const struct tach_rts5918_config *const cfg = dev->config;
	struct tach_rts5918_data *const data = dev->data;
	...
	if (WAIT_FOR(tach->status & TACHO_STS_CNTRDY, PIN_STUCK_TIMEOUT, NULL)) {
		...
		if (tach->status & TACHO_STS_CNTRDY) {
			...
			prev_trash = data->count;
		}
	} else {
		data->count = prev_trash;
	}

	return 0;
}

static int tach_rts5918_channel_get(const struct device *dev, enum sensor_channel chan,
...
static int tach_rts5918_init(const struct device *dev)
{
	const struct tach_rts5918_config *const cfg = dev->config;
	volatile struct tacho_regs *const tach = cfg->regs;
	int ret;
	...
	if (ret != 0) {
		LOG_ERR("rts5918 Tachometer clock control failed (%d)", ret);
		return ret;
	}
	...
	if (ret != 0) {
		LOG_ERR("rts5918 pinctrl failed (%d)", ret);
		return ret;
	}
#endif
...
static const struct sensor_driver_api tach_rts5918_driver_api = {
	.sample_fetch = tach_rts5918_sample_fetch,
	.channel_get = tach_rts5918_channel_get,
};

#define REALTEK_TACH_INIT(inst)                                                                    \
	PINCTRL_DT_INST_DEFINE(inst);                                                              \
                                                                                                   \
	static const struct tach_rts5918_config tach_cfg_##inst = {                                \
		...
		.pulses_per_round = DT_INST_PROP(inst, pulses_per_round),                          \
                                                                                                   ...
                                                                                                   \
	static struct tach_rts5918_data tach_data_##inst;                                          \
                                                                                                   \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, tach_rts5918_init, NULL, &tach_data_##inst,             \
				     &tach_cfg_##inst, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY,   \
				     &tach_rts5918_driver_api);
