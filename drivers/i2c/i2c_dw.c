 ...
 */
void i2c_dw_register_recover_bus_cb(const struct device *dev,
...
static void i2c_dw_isr(const struct device *port)
{
	...
	/* Handle case that tx empty interrupt still remains */
	if( (*(uint32_t *)(reg_base + 0x70) == 0x06) &&
		(read_intr_stat(reg_base) & DW_INTR_STAT_TX_EMPTY))
	{
		clear_bit_intr_mask_tx_empty(reg_base);
	}
