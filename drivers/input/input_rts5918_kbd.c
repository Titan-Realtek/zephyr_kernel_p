...
#define DT_DRV_COMPAT realtek_rts5918_kbd
...
LOG_MODULE_REGISTER(input_realtek_rts5918_kbd, CONFIG_INPUT_LOG_LEVEL);

struct rts5918_kbd_config {
...
struct rts5918_kbd_data {
	struct input_kbd_matrix_common_data common;
};

INPUT_KBD_STRUCT_CHECK(struct rts5918_kbd_config, struct rts5918_kbd_data);

static void rts5918_kbd_drive_column(const struct device *dev, int col)
{
...
static kbd_row_t rts5918_kbd_read_row(const struct device *dev)
{
	const struct rts5918_kbd_config *config = dev->config;
	volatile struct kbm_regs *inst = config->base;
	const struct input_kbd_matrix_common_config *common = &config->common;
	const uint32_t ksi_mask = BIT_MASK(common->row_size);
	uint32_t temp = inst->scan_in;
	uint32_t ksi4_sts = (*(volatile uint32_t *)(0x40230170) & 0x8) >> 3;
	uint32_t ksi5_sts = (*(volatile uint32_t *)(0x40230174) & 0x8) >> 3;
	temp &= ~(0x1 << 4);
	temp &= ~(0x1 << 5);
	temp |= (ksi4_sts << 4);
	temp |= (ksi5_sts << 5);
	/* Bits are active-low, so toggle it (return 1 means key pressed) */
	return (temp ^ ksi_mask);
}

static void rts5918_intc_isr_clear(const struct device *dev)
{
	const struct rts5918_kbd_config *config = dev->config;
	volatile struct kbm_regs *inst = config->base;
...
static void rts5918_kbd_isr(const struct device *dev)
{
	const struct rts5918_kbd_config *config = dev->config;
	volatile struct kbm_regs *inst = config->base;	
	
	uint32_t temp = inst->int_en;
	inst->int_en = 0;
#ifdef CONFIG_SOC_RTS5918
	inst->scan_ctrl = 0;
	inst->scan_status = 0x1FFFFF;
#endif	
...
static void rts5918_kbd_set_detect_mode(const struct device *dev, bool enable)
{
	...
	if (enable) {
		/* W/C interrupt status of KSI pins */
		rts5918_intc_isr_clear(dev);
		*(volatile uint32_t *)(0x40230170) |= (0x1 << 31);
...
static int rts5918_kbd_init(const struct device *dev)
{
	...
	rts5918_kbd_set_detect_mode(dev, false);
	...
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority),
		    rts5918_kbd_isr, DEVICE_DT_INST_GET(0), 0);
...
static const struct input_kbd_matrix_api rts5918_kbd_api = {
	.drive_column = rts5918_kbd_drive_column,
	.read_row = rts5918_kbd_read_row,
	.set_detect_mode = rts5918_kbd_set_detect_mode,
};

static const struct rts5918_kbd_config rts5918_kbd_cfg_0 = {
	.common = INPUT_KBD_MATRIX_DT_INST_COMMON_CONFIG_INIT(0, &rts5918_kbd_api),
...
static struct rts5918_kbd_data rts5918_kbd_data_0;

DEVICE_DT_INST_DEFINE(0, &rts5918_kbd_init, PM_DEVICE_DT_INST_GET(0), &rts5918_kbd_data_0,
		      &rts5918_kbd_cfg_0, POST_KERNEL, CONFIG_INPUT_INIT_PRIORITY, NULL);
...
BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT) == 1,
	     "only one realtek,rts5918-kbd compatible node can be supported");
