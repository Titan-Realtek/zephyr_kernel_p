...
#ifndef _REALTEK_RTS5918_REG_SPIC_H
#define _REALTEK_RTS5918_REG_SPIC_H
...
struct reg_spic_reg {
	...
	uint32_t RFS; 		// READ_FAST_SINGLE		(0xE0)
	uint32_t RDD; 		// READ_DUAL_DATA		(0xE4)
	uint32_t RDAD;		// READ_DUAL_ADDR_DATA	(0xE8)
	uint32_t RQD;		// READ_QUAD_DATA       (0xEC)
	uint32_t RQAD;		// READ_QUAD_ADDR_DATA  (0xF0)
	uint32_t WS;		// WRITE_SINGLE         (0xF4)
	uint32_t WDD;		// WRITE_DUAL_DATA      (0xF8)
	uint32_t WDAD;		// WRITE_DUAL_ADDR_DATA (0xFC)
	uint32_t WQD;		// WRITE_QUAD_DATA      (0x100)
	uint32_t WQAD;		// WRITE_QUAD_ADDR_DATA (0x104)
	uint32_t WE;		// WRITE_ENABLE         (0x108)
	uint32_t RS;		// READ_STATUS          (0x10C)
	uint32_t CTRLR2;
	uint32_t FBAUD;
	uint32_t USERLENGTH;
	uint32_t AUTOLENGTH;
	uint32_t VALIDCMD;	// VALID_CMD            (0x120)
	const uint32_t FLASH_SIZE;
	uint32_t FLUSH;
	const uint32_t DUM_BYTE;
	uint32_t TXNDF;
};

#define SPIC_REG_OFFSET_RQD (0x0EC)
#define SPIC_REG_OFFSET_WQD (0x100)
#define SPIC_REG_OFFSET_FBAUD (0x114)
#define SPIC_REG_OFFSET_VALIDCMD (0x120)

/* Validate SPIC's register field offset is correct */

/* For SPIC auto mode functionality */
_Static_assert(offsetof(struct reg_spic_reg, RQD) == SPIC_REG_OFFSET_RQD);
_Static_assert(offsetof(struct reg_spic_reg, WQD) == SPIC_REG_OFFSET_WQD);
_Static_assert(offsetof(struct reg_spic_reg, FBAUD) == SPIC_REG_OFFSET_FBAUD);
_Static_assert(offsetof(struct reg_spic_reg, VALIDCMD) == SPIC_REG_OFFSET_VALIDCMD);
...
/* AUTOLENGTH */
#define SPIC_AUTOLENGTH_RDDUMMYLEN_Pos (0UL)
#define SPIC_AUTOLENGTH_RDDUMMYLEN_Msk GENMASK(11, 0)
#define SPIC_AUTOLENGTH_ADDRLEN_Pos    (16UL)
#define SPIC_AUTOLENGTH_ADDRLEN_Msk    GENMASK(17, 16)
/* FLUSH */
...
/* VALID_CMD */
#define SPIC_AUTO_VALIDCMD_FRD_SINGLE  BIT(0UL)
#define SPIC_AUTO_VALIDCMD_RD_QUAD_O   BIT(3UL)      // quad data read          	a.k.a (1-1-4)
#define SPIC_AUTO_VALIDCMD_RD_QUAD_IO  BIT(4UL)      // quad data/address read  	a.k.a (1-4-4)
#define SPIC_AUTO_VALIDCMD_WR_QUAD_I   BIT(7UL)      // quad data write         	a.k.a (1-1-4)
#define SPIC_AUTO_VALIDCMD_WR_QUAD_II  BIT(8UL)      // quad data/address write 	a.k.a (1-4-4)
#define SPIC_AUTO_VALIDCMD_DUM_EN      BIT(13UL)     // Enable to push one dummy byte (DUM_BYTE_VAL) 
													 // after pushing address to Flash in auto read
/* READ_QUAD_DATA */
#define SPIC_READ_QUAD_DATA_RD_QUAD_O_CMD_Pos (0UL)
#define SPIC_READ_QUAD_DATA_RD_QUAD_O_CMD_Msk GENMASK(7, 0)

/* WRITE_QUAD_DATA */
#define SPIC_READ_QUAD_DATA_WR_QUAD_I_CMD_Pos (0UL)
#define SPIC_READ_QUAD_DATA_WR_QUAD_I_CMD_Msk GENMASK(7, 0)

/*  */

#endif /* _REALTEK_RTS5918_REG_SPIC_H */
